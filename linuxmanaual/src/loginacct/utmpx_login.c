...
int main(int argc,char *argv[])
{
  ...
  if(argc<2||strcmp(argv[1],"--help")==0)
    usageErr("%s uaername [sleep-time]\n",argv[0]);
  ...
  /* Set ut_line and ut_id based on the terminal associated with
     ...
     The "/dev/ dirname si 5 characters;the "[pt]t[sy]" filename
  ...
  printf("Create login entries in utmp and wtmp\n");
  ...
  memset(&ut.ut_user,0,sizeof(ut.ut_user)); /* Logout record
					       has null username*/
  printf("Create logout entries in utmp and wtmp\n");
  setutxent();
  if(pututxline(&ut)==NULL)	/* Overwrite previous utmp recoud */
    errExit("pututxline");
