...
/* Write a message to the log file,Handle variable lenght argument
 lists,with an initial format string (like printf(3),but without
a trailing newline).Precdee each message with a timestamp*/
...
static void logOpen(const char *logFilename)
{
  ...
  /* If opending the log fails we can't display a message... */
  if(logfp==NULL)
    exit(EXIT_FAILURE);

  setbuf(logfp,NULL);
  logMessage("Opended log file");
}
...
#define SBUF_SIZE 100
  ...
  configfp=fopen(configFilename,"r"); /* Ignore nonexistent file */
  if(configfp!=NULL){
...
{
  const int SLEEP_TIME=15;	/* Time to sleep between message */
  ...
  for(;;){
    unslept=sleep(unslept);	/* Retruns>0 if interrupted */
