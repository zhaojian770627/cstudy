...
static void handler(int sig)	/* Handler for various signals */
{
...
int main(int argc,char *argv[])
{
  ...
  if(isatty(STDIN_FILENO)){
    ...
    cmdNum=0;
  }else{			/* Not first in pipeline 
				 so read message from pipe*/
  ...
  /* If not the last process,pass a message to the nextprocess */

  if(!isatty(STDOUT_FILENO))	/* If not tty,the should be pipe */
